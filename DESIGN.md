...
**Correction, 12 Sep 2026 — `userAnswerTranscript` was logging the wrong utterance.** Voice mode wrote whichever transcription arrived last, and on any card that reaches a rating that is the *rating*: a column of "Good" and "Again" where the answer should be. The field's purpose above — *why does this card keep failing* — was defeated from the moment transcription was switched on, and nothing on the row said so. Fixed to take the first utterance of the card, with `schemaVersion` bumped to 2 so the two populations are tellable apart. The old rows cannot be repaired: the audio is gone by design (§5.1) and the transcripts were never stored. This is the second time a field has been quietly wrong rather than missing, which is an argument for reading a few real rows after any change to what writes them — a null is visible and a plausible wrong value is not.
